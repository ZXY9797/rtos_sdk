...
struct gpio_stm32_config {
	...
	/* IO port */
	int port;
    const struct device *clock_control_dev;
...
static int gpio_stm32_clock_request(const struct device *dev, bool on)
{
	...
	if (on) {
		ret = clock_control_on(cfg->clock_control_dev,
					(clock_control_subsys_t)&cfg->pclken);
	} else {
		ret = clock_control_off(cfg->clock_control_dev,
					(clock_control_subsys_t)&cfg->pclken);
	}
	...
	static const struct gpio_stm32_config gpio_stm32_cfg_## __suffix = {   \
		...
		COND_CODE_1(DT_NODE_HAS_PROP(__node, clocks),		       \
			   (.pclken = { .bus = __bus, .enr = __cenr }, \
				.clock_control_dev = DEVICE_DT_GET(DT_CLOCKS_CTLR(__node))),       \
