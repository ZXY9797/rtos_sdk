...
constexpr uint32_t kFlashUnlockKey1 = 0x45670123U;
constexpr uint32_t kFlashUnlockKey2 = 0xCDEF89ABU;
...
static uint32_t flash_bank_size() {
    // STM32 CMSIS obtains this value from the factory-programmed Flash-size
    // register, so it is intentionally evaluated at runtime.
    return static_cast<uint32_t>(FLASH_BANK_SIZE);
}

static bool address_range_valid(uint32_t addr, uint32_t length) {
    const uint32_t bank_size = flash_bank_size();
    return length != 0U && bank_size >= length
        && addr >= STM32_FLASH_BASE
        && (addr - STM32_FLASH_BASE) <= (bank_size - length);
}
...
static Status stm32_write_block(void *ctx, uint32_t addr,
    ...
    if (ctx == nullptr || data == nullptr || len != STM32_WRITE_BLOCK
        || (addr % STM32_WRITE_BLOCK) != 0U
        || !address_range_valid(addr, STM32_WRITE_BLOCK)) {
...
static Status stm32_erase_sector(void *ctx, uint32_t addr) {
    if (ctx == nullptr
        || (addr % STM32_SECTOR_SIZE) != 0U
        || !address_range_valid(addr, STM32_SECTOR_SIZE)) {
        return Status::InvalidArgument;
    }
    auto *regs = static_cast<FlashRegs *>(ctx);
    uint32_t sector =
...
static void stm32_unlock(void *ctx) {
    ...
    if (regs->CR1 & CR1_LOCK) {
        regs->KEYR1 = kFlashUnlockKey1;
        regs->KEYR1 = kFlashUnlockKey2;
    }
}
...
Flash flash_create_default() {
    ...
    return Flash(STM32_FLASH_BASE, flash_bank_size(),
                    STM32_WRITE_BLOCK,
