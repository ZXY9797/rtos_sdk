...
MotionPlanner::AxisState MotionPlanner::plan_axis(
    float error, float sample_period_s,
    const AxisLimits &limits, const AxisState &state)
{
    const float braking_speed = std::sqrt(
        2.0F * limits.maximum_deceleration * std::abs(error));
    const float target_speed = std::copysign(
        std::min(limits.maximum_speed, braking_speed), error);
    const float speed_error = target_speed - state.rate;
    const bool braking = target_speed * state.rate < 0.0F
        || std::abs(target_speed) < std::abs(state.rate);
    const float acceleration_limit = braking
            ? limits.maximum_deceleration
            : limits.maximum_acceleration;
    const float requested_acceleration = math::clamp(
        speed_error / sample_period_s,
        -acceleration_limit, acceleration_limit);
    const float maximum_acceleration_change =
        limits.maximum_jerk * sample_period_s;
    const float acceleration = math::clamp(
        requested_acceleration,
        state.acceleration - maximum_acceleration_change,
        state.acceleration + maximum_acceleration_change);
    const float rate = math::clamp(
        state.rate + acceleration * sample_period_s,
                                      ...
                                      uint32_t publish_time_us)
{
    ...
    for (size_t axis = 0U; axis < kAxisCount; ++axis) {
        ...
        const AxisState next = plan_axis(
            component(error, axis), sample_period_s, axis_limits, state);
        set_component(rate_, axis, next.rate);
        set_component(acceleration_, axis, next.acceleration);
    }
