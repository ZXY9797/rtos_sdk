...
constexpr float kMinimumDecouplingDeterminant = 1.0e-4F;
...
[[nodiscard]] algo::BiquadConfig notch_config(
    const AxisControllerConfig &config, float sample_frequency)
{
    return {
        algo::BiquadType::Notch,
        sample_frequency,
...
[[nodiscard]] algo::BiquadConfig low_pass_config(
    const AxisControllerConfig &config, float sample_frequency)
{
    return {
        algo::BiquadType::LowPass,
        sample_frequency,
        config.low_pass_frequency_hz,
        kLowPassQualityFactor,
    };
}

[[nodiscard]] algo::LeadLagConfig lead_config(
    const AxisControllerConfig &config, float sample_frequency)
{
    const float gain = config.lead_zero_frequency_hz > 0.0F
...
[[nodiscard]] bool finite_axis_config(
    const AxisControllerConfig &config, float sample_frequency)
{
    const bool notch_valid = config.notch_frequency_hz == 0.0F
        || algo::valid_biquad_config(
            notch_config(config, sample_frequency));
    ...
    const bool lead_valid = lead_bypassed
        || algo::valid_lead_lag_config(
            lead_config(config, sample_frequency));
    return std::isfinite(config.attitude_gain)
        ...
        && algo::valid_biquad_config(
            low_pass_config(config, sample_frequency));
}
...
bool valid_controller_config(const ControllerConfig &config)
{
    ...
    return std::abs(math::determinant(config.decoupling_matrix))
        > kMinimumDecouplingDeterminant;
}
...
bool TwoDegreeController::configure_filters()
{
    for (size_t axis = 0U; axis < kAxisCount; ++axis) {
        ...
        AxisFilters &axis_filters = filters_[axis];
        if (axis_config.notch_frequency_hz == 0.0F) {
            axis_filters.notch.bypass();
        } else if (!axis_filters.notch.configure(
                       notch_config(axis_config,
                                    config_.sample_frequency_hz))) {
            return false;
        }
        if (axis_config.lead_zero_frequency_hz == 0.0F
            && axis_config.lead_pole_frequency_hz == 0.0F) {
            axis_filters.lead.bypass();
        } else if (!axis_filters.lead.configure(
                       lead_config(axis_config,
                                   config_.sample_frequency_hz))) {
            return false;
        }
        if (!axis_filters.low_pass.configure(
                low_pass_config(axis_config,
                                config_.sample_frequency_hz))) {
            return false;
        }
    }
    ...
    const ControllerInput &input, Vector3 &feedback)
{
    ...
    for (size_t axis = 0U; axis < kAxisCount; ++axis) {
        ...
        if (!filters_[axis].notch.update(rate_error, shaped_error)
            || !filters_[axis].lead.update(shaped_error, shaped_error)
            || !filters_[axis].low_pass.update(
