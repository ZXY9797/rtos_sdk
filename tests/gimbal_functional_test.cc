...
[[nodiscard]] bool test_algorithm_primitives()
{
    static_assert(std::is_same_v<gimbal::Vector3, algo::Vector3>);
    algo::BiquadFilter biquad;
    float filtered = 0.0F;
    const algo::BiquadConfig invalid_biquad {
        algo::BiquadType::LowPass, 0.0F, 20.0F, 0.707F,
    };
    if (biquad.configure(invalid_biquad)
        || biquad.update(1.0F, filtered)) {
        return false;
    }
    biquad.bypass();
    if (!biquad.update(0.25F, filtered)
        || !near(filtered, 0.25F, 1.0e-6F)) {
        return false;
    }
    const algo::BiquadConfig notch {
        algo::BiquadType::Notch, 1000.0F, 80.0F, 3.0F,
    };
    if (!biquad.configure(notch)
        || !biquad.update(1.0F, filtered)
        || !std::isfinite(filtered)) {
        return false;
    }
    algo::LeadLagCompensator lead;
    const algo::LeadLagConfig lead_config {
        1000.0F, 20.0F, 80.0F, 4.0F,
    };
    if (!lead.configure(lead_config)
        || !lead.update(0.5F, filtered)
        || !std::isfinite(filtered)) {
        return false;
    }
    const algo::MotionProfileLimits limits {2.0F, 8.0F, 10.0F, 80.0F};
    const algo::MotionProfileState initial {};
    algo::MotionProfileState next {};
    return !algo::JerkLimitedMotionProfile::step(
               1.0F, 0.0F, limits, initial, next)
        && algo::JerkLimitedMotionProfile::step(
            1.0F, 0.001F, limits, initial, next)
        && next.acceleration > 0.0F
        && next.acceleration <= 0.080001F
        && next.rate > 0.0F && next.rate <= 2.0F;
}
...
[[nodiscard]] bool test_voltage_foc()
{
    ...
    const foc::PhaseDuty neutral = foc.calculate(0.0F, 0.0F, 0.0F, 0.0F);
    const foc::PhaseDuty drive = foc.calculate(0.3F, 0.0F, 30.0F, 12.0F);
    const foc::PhaseDuty overflow = foc.calculate(
...
[[nodiscard]] bool test_attitude_ekf()
{
    ...
    if (!rotated_filter.configure({}, rotated_calibration)) {
        return false;
    }
    ...
    if (!rotated_filter.predict(sample, 0.001F)) {
        return false;
    }
...
[[nodiscard]] bool test_safety_and_shared_memory()
{
    ...
    ipc::SnapshotTopic<gimbal::SafetyOutput> topic;
...
[[nodiscard]] bool test_shared_memory_concurrency()
{
    ...
    ipc::SnapshotTopic<SharedTestValue> topic;
...
int main()
{
    ...
    const TestCase tests[] {
        {"algorithm_primitives", test_algorithm_primitives},
